...
public abstract class AbstractSpoutSender : MonoBehaviour {
    ...
    private const string CLEAR_ALPHA_KEYWORD = "CLEAR_ALPHA";
    ...
    private void SendRenderTexture(Texture source) {
        ...
        if (alphaSupport) {
            _blitMaterial.DisableKeyword(CLEAR_ALPHA_KEYWORD);
        } else {
            _blitMaterial.EnableKeyword(CLEAR_ALPHA_KEYWORD);
        }

        RenderTexture tempRT = RenderTexture.GetTemporary(_sharedTexture.width, _sharedTexture.height);
    ...
    private void InitializePlugin(int width, int height) {
        ...
        {
            Plugin.Logger.LogError("Failed to initialize! Spout may not be ready.");
    ...
    private void InitializeSharedTexture() {
        ...
        IntPtr ptr = PluginEntry.GetTexturePointer(_plugin);
